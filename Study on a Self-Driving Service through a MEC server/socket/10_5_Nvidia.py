...
def main():
    ...
    model_path = '/home/pi/AI_CAR/lane_navigation_final(Nvidia).h5'
    ...
    try:
        with open('socket_dalay.txt', 'w') as f:
            sys.stdout = f
            while True:
                keyValue = cv2.waitKey(1) # default -1 / 0.001 seconds
                # print(keyValue)
                if keyValue == ord('q') :
                    break
                elif keyValue == 82 : 
                ...
                # steering_angle_socket = clientSocket.recv(1024)
                # steering_angle = steering_angle_socket.decode()
